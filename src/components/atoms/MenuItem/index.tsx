...
export default function MenuItem({ label, icon, onClickRedirectUrl }: MenuItemProps) {
  ...
  return (
    <div className="relative w-[394px] h-[209px] border-none rounded-lg overflow-hidden shadow-lg transition-transform duration-300 hover:scale-105">
      <img
      ...
      />
      <div className="absolute bottom-1 left-1 right-1 flex justify-between items-center bg-gradient-to-r from-[#ef4444] via-[#ef4444] to-[#892727] bg-opacity-50 backdrop-blur border border-white/20 rounded-lg p-2 shadow-sm animate-fadeInUp">
        <div
          className="flex flex-row gap-3 w-[394px] cursor-pointer hover:animate-pulse items-center active:scale-95 transition-transform"
          onClick={() => router.push(onClickRedirectUrl)}
        >
          <div className="w-[80px] h-[80px] bg-red-50 rounded-2xl drop-shadow-lg flex items-center justify-center hover:brightness-125 transition-all">
